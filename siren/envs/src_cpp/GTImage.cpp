...
GTImage::GTImage(std::string& fileName, cv::Size cellSize, cv::Size pixelSize) {
    ...
    this->cellImage = cv::Mat::zeros(cv::Size(pixelSize.width/cellSize.width, pixelSize.height/cellSize.height), CV_8UC3);
    for (int i  = cellSize.height / 2; i < pixelImage.rows; i += cellSize.height) {
        for (int j = cellSize.width / 2; j < pixelImage.cols; j += cellSize.width) {
            ...
            if (value == cv::Vec3b(0, 0, 0)) {
                cellImage.at<cv::Vec3b>(i/cellSize.height, j/cellSize.width)[0] = 0;   // label 0 for nothing (black pixels, including non-centerlines and unknown labels)
